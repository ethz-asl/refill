...
def check_modified_after_staging(staged_files):
    ...
    _, unstaged_changes = subprocess.getstatusoutput("git diff --name-only")
...
def check_python_lint(repo_root, staged_files):
    ...
    for changed_file in staged_files:
        if re.search(r"\.py$", changed_file):
            ...
            pylint_args = [
                "--rcfile=" + repo_root + "/devtools/pylint.rc",
                "-rn",
...
def main():
    ...
    _, output = subprocess.getstatusoutput("git diff --staged --name-only")
